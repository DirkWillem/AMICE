...
void wifi::AccessPoint::OnWifiEvent(
    ...
    switch(event_id) {
        ...
        case SYSTEM_EVENT_STA_GOT_IP: {
            ...
            ESP_LOGI(Tag, "[STA_GOT_IP] got ip: %s", ip4addr_ntoa((ip4_addr_t*)&got_ip->ip_info.ip));
            xEventGroupSetBits(self->m_wifi_event_group, BIT0);
            break;
        }
        case WIFI_EVENT_AP_STACONNECTED: {
            const auto sta_connected = reinterpret_cast<wifi_event_ap_staconnected_t*>(event_data);
        ...
        case SYSTEM_EVENT_AP_STADISCONNECTED: {
            const auto sta_disconnected = reinterpret_cast<wifi_event_ap_stadisconnected_t*>(event_data);
